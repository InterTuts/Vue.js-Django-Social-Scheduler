...
onMounted(async () => {
  ...
  try {
    // Send request
    const response: AxiosResponse<ApiResponse<null>> = await axios.get(
      'api/networks/callback/' + network + '?code=' + code,
    );
        ...
        <v-row>
          <v-col cols="12">
            <div class="networks-form-alerts">
              <div
                class="networks-form-alert-success top-to-bottom-animation"
                role="alert"
                v-if="successMessage"
              >
                <span
                  class="mdi mdi-bell-outline networks-form-alert-success-icon"
                ></span>
                <p>{{ successMessage }}</p>
              </div>
              <div
